...
def ComputeOverlay(picture1,picture2):
    ...
    for i in range(bound_x):
        if i%skip==0:
            for j in range(bound_y):
                if j%skip==0 :
                    if nspr_v[i,j,(i+Mx)%bound_x,(j+My)%bound_y]!=0:
                        tmp_nspr=nspr_v[i,j,(i+Mx)%bound_x,(j+My)%bound_y]
                    else:
                        pict_one=PICT(picture1,i,j,W,H)
                        pict_two=PICT(picture1,i+Mx,j+My,W,H)
                        tmp_nspr=NSPR(pict_one,pict_two)
                    if tmp_nspr>tao:
                        pict1=PICT(picture1,i,j,W,H,Mx,My)
                        result.append(pict1)
                        effective_blocks+=1
    print('effective:',effective_blocks)
    print('total number:',bound_x*bound_y)
    ...
    if(dens>=rate):
        print('dens:',dens)
        #print('1    left:',left,'top:',top,'right:',right,'down:',down)
    ...
    if(dens>=rate):
        print('dens:',dens)
        #print('2    left:',left,'top:',top,'right:',right,'down:',down)
        return np.array([left,top,(right-left+1)*W,(down-top+1)*H,Mx,My])
    [left,top]=[0,0]
    [right,down]=[bound_x-1,bound_y-1]
    while(dens<rate):
        if right>0:
            #print('right dens:',dens)
            #print('left:',left,'top:',top,'right:',right,'down:',down)
            right-=1
        else:
            break
    ...
    if(dens>=rate):
        print('dens:',dens)
        #print('3    left:',left,'top:',top,'right:',right,'down:',down)
        return np.array([left,top,(right-left+1)*W,(down-top+1)*H,Mx,My])
    [left,top]=[0,0]
    [right,down]=[bound_x-1,bound_y-1]
    while(dens<rate):
        if down>0:
            #print('down dens:',dens)
            #print('left:',left,'top:',top,'right:',right,'down:',down)
            down-=1
        else:
            break
    ...
    print('dens:',dens)
    #print('4    left:',left,'top:',top,'right:',right,'down:',down)
    return np.array([left,top,(right-left+1)*W,(down-top+1)*H,Mx,My])
